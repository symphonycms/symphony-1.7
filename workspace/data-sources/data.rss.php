<?php
	...
	Class dsRss Extends DataSource{

		var $_dsFilterENCODE = 'yes';
		var $_dsFilterLIMIT = '10';
		var $_dsFilterSORT = 'desc';
		var $_dsFilterFORMAT_TYPE = 'list';
		...
		function about(){
			return array(
						 ...
						 'version' => '1.0',
						 'release-date' => '2006-10-04 01:02:07');
		}
		...
		function grab($param=array()){
			...
			$sql = "SELECT DISTINCT t1.id "
				 ...
				 . "WHERE t8.section_id = '$section_id' " . $where
				 . "GROUP BY t1.id "
				 . "ORDER BY t1.`publish_date_gmt`";

			$kTotalEntryCount = count($this->_db->fetchCol('id', $sql));
			...
			if(empty($entries) || !is_array($entries)){
				...
				if(in_array("pagination-info", $this->_dsFilterXMLFIELDS)){
					...
					$pageinfo->setAttribute("total-entries", $kTotalEntryCount);
					$pageinfo->setAttribute("total-pages", $kTotalPages);
					$pageinfo->setAttribute("entries-per-page", $limit);
						...
						foreach($fields as $f){
							...
							if(@in_array($f['field_handle'], $this->_dsFilterXMLFIELDS)){
								...
								elseif($f['type'] == 'foreign'){
									...
									$section_id = $f['foreign_section'];
									$section_handle = $this->_db->fetchVar('handle', 0, "SELECT `handle` FROM `tbl_sections` WHERE `id` = '$section_id ' LIMIT 1");
									$newField->setAttribute("handle", $f['handle']);
									$newField->setAttribute("type", 'foreign');
									$newField->setAttribute("section-id", $section_id);
									...
									foreach($f['value_raw'] as $h){
										$entry_id = $entryManager->fetchEntryIDFromPrimaryFieldHandle($section_id, $h);
